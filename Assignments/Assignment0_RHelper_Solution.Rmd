...
mydata %>%
  filter(category_1 == "Deaths where COVID-19 was mentioned on the death certificate (ICD-10 U07.1 and U07.2)") %>% 
  group_by(week_no) %>% 
  summarise(mean_deaths = mean(counts)) %>%
  ggplot(aes(x=week_no, y=mean_deaths)) + geom_point() + geom_smooth(method="lm") + labs(x="Week Number", y = "Average COVID Deaths") + theme_minimal()
