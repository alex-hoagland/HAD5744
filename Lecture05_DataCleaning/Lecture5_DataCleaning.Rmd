...
data$EXPOTOT <- data$EXPTOT / 0.73 # USD to CAD
data$EXPTOT <- ifelse(data$EXPTOT > 250000, 250000, data$EXPTOT) # topcode
hist(data$EXPTOT)
data <- data %>% mutate(logEXPTOT = log(EXPTOT+1)) # log-transform for analysis
hist(data$logEXPTOT)
data$logEXPTOT %>% summary() # did I need a +1 here? What does that tell us? 

# What about another commonly used outcome: self-reported health? 
data$HEALTH # how would you recommend we deal with this? 
data <- data %>% mutate(srh1 = ifelse(HEALTH > 0 & HEALTH <= 2, 1, 
                                      ifelse(HEALTH > 2 & HEALTH < 7, 0, NA)), 
                        srh2 = ifelse(HEALTH > 0 & HEALTH < 7, HEALTH, NA)) # what do these do? 
...
cor(data$healthshock, data$logEXPTOT) # There is a positive correlation -- is it informative enough? 
...
ggplot(data %>% filter(healthshock == 1), aes(x = Age.adjusted.Death.Rate, y = logEXPTOT)) + geom_point() + theme_minimal() # What do we see here? Does it make sense? 

# What about a binned scatterplot
ggplot(data %>% filter(healthshock == 1), aes(x = Age.adjusted.Death.Rate, y = logEXPTOT)) + 
  geom_point(alpha=0.1) + 
  stat_summary_bin(fun=mean, bins=30, color="red", size=1) + 
  theme_minimal() + 
  labs(title="Binned Scatterplot of Spending vs. Cancer Death Rate", 
       x="Age-Adjusted Cancer Death Rate (per 100,000)", 
       y="Log(Total Annual Spending)") +
  theme(plot.title = element_text(hjust = 0.5))
``` 

## Constructing an Analytic Dataset
Now, we're ready to do some analysis! We have two cohorts we want to look at -- those affected by health shocks and their spouses. Let's construct two datasets for these two cohorts.  
...
cohort1 %>% ggplot(aes(x = logEXPTOT, fill = factor(treated))) + 
...
logmean1 <- cohort1 %>% ungroup() %>% filter(treated=="Health Shock") %>% 
  summarize(mean = mean(logEXPTOT,na.rm=T)) %>% as.numeric()
logmean0 <- cohort1 %>% ungroup() %>% filter(treated=="No Health Shock") %>% 
  summarize(mean = mean(logEXPTOT,na.rm=T)) %>% as.numeric()
mean1 <- exp(logmean1)-1
mean0 <- exp(logmean0)-1
fig1 <- cohort1 %>% ggplot(aes(x = logEXPTOT, fill = factor(treated))) + 
...
logmean1 <- cohort2 %>% ungroup() %>% filter(treated=="Spouse Health Shock") %>% 
  summarize(mean = mean(logEXPTOT,na.rm=T)) %>% as.numeric()
logmean0 <- cohort2 %>% ungroup() %>% filter(treated=="No Health Shock") %>% 
  summarize(mean = mean(logEXPTOT,na.rm=T)) %>% as.numeric()
mean1 <- exp(logmean1)-1
mean0 <- exp(logmean0)-1
fig2 <- cohort2 %>% ggplot(aes(x = logEXPTOT, fill = factor(treated))) + 
  geom_density(alpha = 0.5) + theme_classic() + 
  geom_vline(xintercept = logmean1, linetype = "dashed", color = "blue") +
  geom_vline(xintercept = logmean0, linetype = "dashed", color = "red") +
  annotate("text", x = logmean1+1.8, y = 0.3, label = paste("Median: $",round(mean1,0))) +
...
fig3 <- cohort1 %>% ungroup() %>% mutate(healthshock_none = ifelse(treated=="No Health Shock",1,0),
                                 healthshock_any = ifelse(treated =="Health Shock", 1, 0)) %>% 
  select(-c("healthshock")) %>% 
  select(DUID,FAMIDYR,starts_with("healthshock"),logEXPTOT) %>% 
  pivot_longer(cols=starts_with("healthshock")) %>% # summary()
  filter(value == 1) %>% # keep spending in each category only if it is equal to 1
  ...
  ggplot(aes(x = name, y = logEXPTOT, fill = name)) + 
    geom_boxplot() + theme_classic() + 
  labs(x="", y="", fill ="Own Health Shock") + 
  theme(axis.text.x=element_text(angle=-30))

fig4 <- cohort2 %>% ungroup() %>% mutate(healthshock_none = ifelse(treated=="Spouse Health Shock",1,0),
                                 healthshock_any = ifelse(treated =="Health Shock", 1, 0)) %>% 
  select(-c("healthshock")) %>% 
  select(DUID,FAMIDYR,starts_with("healthshock"),logEXPTOT) %>% 
  pivot_longer(cols=starts_with("healthshock")) %>% # summary()
