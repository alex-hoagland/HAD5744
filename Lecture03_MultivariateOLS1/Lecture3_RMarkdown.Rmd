...
Let's look at a simple summary statistics table here: 
```{r summary}
mydata %>%
  group_by(croploss) %>%
  summarise(
    ...
    sd_income     = sd(income),
    mean_health   = mean(health), # what is this measuring? How do we know it's a "good" measure?
    sd_health     = sd(health)
  ) # how would you convert this into a nice table? 
```

What if we try to visualize the differences in income and health across both groups? What story does this tell us? 
```{r visualize}
# Show how average income and average health differs over croploss, in a bar graph
mydata %>%
  group_by(croploss) %>%
  summarise(mean_income = mean(income),
            mean_health = mean(health)) %>%
  pivot_longer(cols = c(mean_income, mean_health), 
               names_to = "variable", 
               values_to = "value") %>%
  ggplot(aes(x = factor(croploss), y = value, fill = variable)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Crop Loss Shock (0=No, 1=Yes)", 
       y = "Average Value", 
       fill = "Variable") +
  theme_minimal()
...
```{r unorganized}
mydata <- mydata %>% mutate(randomshock = runif(nrow(mydata))) 
mydata <- mydata %>% mutate(indig = ifelse(randomshock>0.85, 1, 0)) 
  # Suppose that 15% of our sample is indigenous
